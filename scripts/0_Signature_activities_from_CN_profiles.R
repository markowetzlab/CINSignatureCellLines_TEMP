...
## Data
BASE="resources/cnsigs2_revisions"
OUT=file.path("data/signatures")
OUTFILEADDON="_ASCAT_cellLines_penalty70"
...
DAT=file.path("data/cellLine_segment_ascat_sc_fixed_purity_tCN.rds")
WIGGLE=0.1
IGNOREDELS=FALSE
CORES=2
...
## Load functions
source(file.path(BASE, "scripts/main_functions.R"))
...
write.table(dfAllSegs, file.path(OUT, paste0("0__absolute_CN", OUTFILEADDON, ".txt")), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
saveRDS(dfAllSegs, file.path(OUT, paste0("0_absolute_CN", OUTFILEADDON, ".rds")) )
...
# Write txt and rds
write.table(dtSmooth, file.path(OUT, paste0("1_absolute_CN_smoothed", OUTFILEADDON, ".txt")), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
saveRDS(dtSmooth, file.path(OUT, paste0("1_absolute_CN_smoothed", OUTFILEADDON, ".rds")) )
...
saveRDS(brECNF, file.path(OUT, paste0("2_ECNF", OUTFILEADDON, ".rds")) )
...
saveRDS(object = allMats, file = file.path(OUT, paste0("3_SxC_Matrix", OUTFILEADDON, ".rds")))
write.table(x = allMats, file = file.path(OUT, paste0("3_SxC_Matrix", OUTFILEADDON, ".txt")), 
            quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")

allMats = t( allMats )
saveRDS(object = allMats, file = file.path(OUT, paste0("3_CxS_Matrix", OUTFILEADDON, ".rds")))
write.table(x = allMats, file = file.path(OUT, paste0("3_CxS_Matrix", OUTFILEADDON, ".txt")), 
            quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
...
saveRDS(object = Hraw, file = file.path(OUT, paste0("4_Exposures_to_TCGA_Signatures_raw", OUTFILEADDON, ".rds")))

H = t( apply(Hraw, 2, function(x) x/sum(x)) )
write.table(x = H, file = file.path(OUT, paste0("4_Exposures_to_TCGA_Signatures", OUTFILEADDON, ".txt")), quote = FALSE, row.names = TRUE, col.names = TRUE, sep = "\t")
saveRDS(object = H, file = file.path(OUT, paste0("4_Exposures_to_TCGA_Signatures", OUTFILEADDON, ".rds")))
